...
public class DemoConsumer extends BaseConsumerWorker {
  ...
  public DemoConsumer(String deviceID) {
    ...
    this.serviceName = "Service_" + deviceID;
  ...
  public void consumeService() {
	  ...
	  else
	  {
		  ...
		  ApisService consumer = new ApisServiceConsumerREST_WS(serviceEndpoint);
		  
		  List<ApisItem> ret = consumer.getAllItems();
		
		  for(ApisItem item : ret)
		  {
  ...
  private String sendOrchestrationRequest(ServiceRequestForm srf) {
   ...
   // System.out.println("Orchestration Response payload: " + Utility.toPrettyJson(null, orchResponse));
    ...
    if(form != null)
    {
        ...
        log.debug("Received provider system URL: " + ub.toString());
